...
- **Red caminera (MOP)** is a fourth static GeoJSON layer at `public/data/red-vial.geojson`, generated by `npm run data:build:red-vial` from the Dirección de Vialidad ArcGIS REST service (rest-sit.mop.gob.cl, the backend of mapas.mop.cl). Violet lines with per-classification visual hierarchy; hover tooltip + popup expose the official road name (`NOMBRE_CAMINO`) and ROL de Vialidad, which often differ from Google/OSM naming.
