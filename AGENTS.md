...
- **Mapa de calor de valor ($/m²)** is a DERIVED layer with no static file and no ETL: `/api/hexbins` aggregates the CBR points themselves in Neon with PostGIS `ST_HexagonGrid` (3.3.3 is installed) and returns one GeoJSON hexagon per cell carrying the **median** of `monto / superficieTerreno`, plus P25/P75 and `n`. Cell edge is derived from the zoom SERVER-SIDE (100 m … 10 km); the `JOIN LATERAL ST_HexagonGrid(edge, pts.g)` runs **per point**, so cost is O(rows in bbox), not O(viewport area). Three rules are non-negotiable: aggregate with the MEDIAN (never the mean — mean/median of `monto` is 5,06× in this base), colour by QUANTILE breaks recomputed over the visible cells (never linear), and always filter by a single `destino` (median $/m² is ~261× higher for `H` than for `A`; a shared scale is meaningless). Opacity encodes CONFIDENCE (`n`), not value. See `docs/plan-mapa-de-calor.md`.
...
| `src/lib/hexbins.ts` | Mapa de calor de valor: zoom→edge ladder, colour ramps, quantile breaks, confidence opacity, destino codes, disclaimer |
