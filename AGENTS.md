...
| `npm run data:build:vegetacional` | Regenerate regional `public/data/vegetacional/*.geojson` from CONAF/IDE Minagri |
...
- **Recursos vegetacionales (CONAF)** is a regional static GeoJSON layer at `public/data/vegetacional/`, generated by `npm run data:build:vegetacional` from the official IDE Minagri SHP downloads. The browser loads only regions intersecting the viewport; public attributes include land use, subuse, vegetation structure/cover, forest typology and dominant scientific/common species names when present. Regional vintages are heterogeneous and are shown explicitly. License/attribution in `src/lib/vegetacional.ts`.
...
| `scripts/build-vegetacional.mjs` | ETL: CONAF/IDE Minagri regional RAR → 7-Zip + WGS84/simplification → regional GeoJSON + meta |
