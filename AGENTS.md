...
| `npm run data:build:bioclima` | Generate bioclimate metadata stub (`public/data/bioclima.meta.json`) from WORLDCLIM investigation |
...
- **Derechos de aprovechamiento de aguas (DGA)** is a PLANNED layer
  (Fase 2): contexto hídrico (glaciares SNIA, cuencas DGA), with link-outs
  to the Catastro Público de Aguas for individual expedientes. Currently
  in source discovery phase (`scripts/build-derechos-agua.mjs` is a stub).
  See `docs/roadmap.md` § 2.2 for feasibility research.
...
- **Bioclima (WorldClim 2.1)** is a HALF-BUILT raster layer (Fase 5.1) — **the toggle
  currently draws nothing on the map**. What exists: the ETL (`npm run data:build:bioclima`)
  downloads the official 628 MiB package (all 19 bioclimatic variables ship in ONE zip —
  there is no per-variable download) and extracts just BIO1 (mean annual temperature, °C
  float32 — v2.1 dropped the ×10 integer scaling of v1.4) and BIO12 (annual precipitation, mm)
  into `.research/worldclim/` (gitignored, 73 MB); `src/lib/bioclima.ts` holds both colour
  ramps and the attribution; `LayersControl` has the row, the variable selector and the legend.
  What is missing: `/api/bioclima/export` still returns a 1×1 transparent PNG placeholder
  instead of rendering the GeoTIFF window, and `MapView.tsx` has NO bioclima code at all, so
  no `L.ImageOverlay` is ever mounted. Climatology is **1970-2000** at 2.5 arc-minutes — an
  interpolated surface from weather stations, never a site measurement. See `docs/roadmap.md`
  § 5.1 for the remaining checklist.
...
| `src/lib/bioclima.ts` | Bioclima (WORLDCLIM) types, color palettes (temp, precip), attribution & layer config |
...
| `scripts/build-bioclima.mjs` | Investigation stub: WORLDCLIM BIO1 (temp) + BIO12 (precip) raster sources, download recipe, metadata |
