...
- **Bioclima (WORLDCLIM v2.1)** is a PLANNED DYNAMIC RASTER layer (Fase 5.1):
  currently in source discovery phase (`scripts/build-bioclima.mjs` generates metadata stub).
  Future implementation: `/api/bioclima/export` will serve temperature + precipitation rasters
  (1981–2010 climatology, 2.5 min ~5 km) as PNG by viewport, styled with divergent palettes
  (blue cold ↔ red hot for temperature; brown arid ↔ blue wet for precipitation). Backbone
  for ecological niche analysis, species distribution, climate change assessment.
  See `docs/roadmap.md` § 5.1 for full description.
